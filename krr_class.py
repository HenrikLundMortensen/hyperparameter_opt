...
class krr_class():
    ...
    def predict_energy(self, fnew=None, pos=None):
        ...
        self.similarityVec = self.comparator.get_similarity_vector(self.fnew,self.featureMat)
    ...
    def getSigmaGradient(self,FMatTrain,FMatVal,Y,Yval):
        ...
        Ktrain = self.comparator.getKernelMat(FMatTrain,FMatTrain)
        Kval = self.comparator.getKernelMat(FMatTrain,FMatVal)

        dKtrainds = self.comparator.getSigmaDerivKernelMat(FMatTrain,FMatTrain)
        dKvalds = self.comparator.getSigmaDerivKernelMat(FMatTrain,FMatVal)
        
        alpha = self.alpha
        dEvalds = -1/Nval*( alpha.T@dKvalds.T@(Yval-Kval@alpha) + (Yval-Kval@alpha).T@dKvalds@alpha)
...
if __name__ == "__main__":
    ...
    plt.figure(1)
    plt.plot(delta_array, Ftestx, color='c', label='F target')
    plt.plot(delta_array, Fpredx, color='y', label='F model')
    plt.plot(delta_array[1:]-dx/2, Ffinite, color='g', linestyle=':', label='F finite diff.')
    plt.plot(delta_array, Etest, label='E target')
